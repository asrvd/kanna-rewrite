...
def check(gid):
    if (
        db.child("CHATBOT").child(gid).child("SETUP").child("set").get().val() == False
        or db.child("CHATBOT").child(gid).child("SETUP").child("set").get().val()
        == None
    ):
        return False
    else:
...
class ChatBot(commands.Cog):
    ...
    async def chat_disable(self, ctx):
        if check(ctx.guild.id):
            ...
            db.child("CHATBOT").child(ctx.guild.id).child("SETUP").child("set").set(
                False
            )
            await ctx.send(
                "`Chat-With-Kanna` disabled for this guild.\nSend `kana chat_enable` to enable it again."
            )
        else:
                    ...
                    async with message.channel.typing():
                        async with aiohttp.ClientSession() as session:
                            async with session.get(
                                f"http://api.brainshop.ai/get?bid={bid}&key={key}&uid={message.author.id}&msg={message.content}"
                            ) as resp:
                                if resp.status == 200:
    ...
    async def chat_setup_error(self, ctx, error):
        if isinstance(error, commands.MissingPermissions):
            await ctx.send(
                "You do not have the permission to do that. Required Perm: `Manage Server`"
            )
        elif isinstance(error, discord.Forbidden):
            await ctx.send(
                "Kanna doesn't have perms to do that. Please make sure kanna has the permission to manage channels."
            )
