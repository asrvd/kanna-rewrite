...
class BookMark(commands.Cog):
    ...
    async def add(self, ctx, message: discord.Message, *, name: str):
        if message is None or name is None:
            await ctx.reply(
                "Invalid Arguments, Example `kana bm add <message_id> <bookmark_name>`"
            )
        else:
            if not check_bookmark(ctx.author.id, name):
                ...
                emb.add_field(
                    ...
                    value=get_unix(message.created_at),
                    inline=True,
                )
                emb.add_field(
                ...
                if len(media) > 0:
                    for i in range(0, len(media)):
                        if i == 0:
                            emb.add_field(
                                name="Medias",
                                value=f"[Attachmet {i+1}]({media[i].url})",
                                inline=False,
                            )
                        else:
                            emb.set_field_at(
                                index=len(emb.fields) - 1,
                                name="Medias",
                                value=emb.fields[-1].value
                                + " | "
                                + f"[Attachmet {i+1}]({media[i].url})",
                                inline=False,
                            )
                set_bookmark(
                    ctx.author.id,
    ...
    async def show(self, ctx, *, name: str):
        if check_bookmark(ctx.author.id, name):
            ...
            emb.add_field(
                name="Jump to Message", value=f"[Click Here]({b['link']})", inline=True
            )
    ...
    async def all(self, ctx, *, user: discord.User = None):
        ...
        bms = (
            list(dict(get_bookmarks(user.id)).keys())
            if get_bookmarks(user.id) is not None
            else []
        )
        print(bms)
        if len(bms) > 0:
...
class BookMarkApp(commands.Cog):
    ...
    async def bookmark_message(self, ctx, message: discord.Message):
        if not check_bookmark(ctx.author.id, msg.content):
            ...
            emb.add_field(name="Bookmark ID", value=message.id, inline=True)
            emb.add_field(
                name="Message Created", value=get_unix(message.created_at), inline=True
            )
            emb.add_field(
                name="Jump to Message",
                value=f"[Click Here]({message.jump_url})",
