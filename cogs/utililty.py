...
class Util(commands.Cog):
    ...
    @slash_command(name="define", description="Define any word using Urban Dictionary.")
    async def define(self, ctx, *, query: str):
        if query == None:
            await ctx.respond(
                "You need to provide a Query to get the defintion!", ephemeral=True
            )
        else:
            async with aiohttp.ClientSession() as cs:
                ...
                async with cs.get(url, params=querystring, headers=headers) as r:
                    ...
                    if list == []:
                        await ctx.respond(
                            "No results available for this query ;-;", ephemeral=True
                        )
                    else:
                        ...
                        emb.set_author(
                            name=f"Definition Of {query.capitalize()}",
    ...
    @slash_command(name="manga", description="Get info about any manga.")
    async def anime(self, ctx, *, name: str):
...
class NUtility(commands.Cog):
    ...
    async def on_message(self, message):
        if checkafk(message.author.id, message.guild.id):
            ...
            await message.reply(
                f"Welcome back! your AFK has been removed", delete_after=10
            )
