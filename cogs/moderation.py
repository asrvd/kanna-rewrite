...
class Mod(commands.Cog):
    ...
    async def mute(
        self, ctx, user: discord.Member = None, time="1m", *, reason: str = None
    ):
        ...
        await ctx.reply(
            f"> `⏲`**{user}** was timed out for **{time[:-1] + ' ' + time_unit[time[-1]]}**.\n> Reason: **{reason}**"
        )
        ...
        if self.client.get_user(int(user_id)) == None:
            ...
            for ban_entry in banned_members:
                ...
                if (u.name, u.discriminator) == (member_name, member_discriminator):
                    ...
                    await user.send(
                        f"You were unbanned from server **{ctx.guild.name}**\nyou can join the server using this link: {link}"
                    )
                else:
    ...
    async def purge_error(self, ctx, error):
        if isinstance(error, commands.CommandOnCooldown):
            await ctx.reply(
                f"Try again in {error.retry_after:.2f} seconds.", delete_after=10
            )
    ...
    async def ban_error(self, ctx, error):
        ...
        if isinstance(error, commands.MissingPermissions):
            ...
            await ctx.reply(
                "Kanna doesn't have the required permissions to use this action! `Ban Members` or  `Administrator` permissions required!"
            )
    ...
    async def kick_error(self, ctx, error):
        ...
        if isinstance(error, commands.MissingPermissions):
            ...
            await ctx.reply(
                "Kanna doesn't have the required permissions to use this action! `Kick Members` or  `Administrator` permissions required!"
            )
    ...
    async def mute_error(self, ctx, error):
        ...
        if isinstance(error, commands.MissingPermissions):
            ...
            await ctx.reply(
                "Kanna doesn't have the required permissions to use this action! `Moderate Members` or  `Administrator` permissions required!"
            )
    ...
    async def lock_error(self, ctx, error):
        ...
        if isinstance(error, commands.MissingPermissions):
            ...
            await ctx.reply(
                "Kanna doesn't have the required permissions to use this action! `Manage Channels` or  `Administrator` permissions required!"
            )
