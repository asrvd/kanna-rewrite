...
class MemeView(View):
    ...
    async def n_callback(self, button, interaction):
        ...
        async with aiohttp.ClientSession() as cs:
            async with cs.get(
                f"https://www.reddit.com/r/{random.choice(ml)}/new.json?sort=hot"
            ) as r:
                res = await r.json()
                # print(res)
                embed.set_image(
...
class Meme(commands.Cog):
    ...
    @slash_command(name="headpat", description="Generates a HeadPat GIF using avatar")
    ...
    @slash_command(
        name="triggered", description="Make a Triggered GIF using any user's avatar"
    )
    ...
    @slash_command(name="gay", description="Turn anyone's avatar to GAY")
    async def gay(self, ctx, user: discord.User = None):
        user = ctx.author if user is None else user
        await ctx.defer()
        asset = w.set_overlay().overlay(
            type="rainbow", image_url=ctx.author.display_avatar.url
        )
        dest = BytesIO(asset)
        dest.seek(0)
    ...
    async def eject(self, ctx, user: discord.User = None):
        ...
        await ctx.defer()
        asset = w.generate().eject(
            url=user.display_avatar.url, text=user.display_name, outcome=sussy
        )
        dest = BytesIO(asset)
        dest.seek(0)
    ...
    async def rip(self, ctx, user: discord.User = None):
        user = ctx.author if user is None else user
        await ctx.defer()
        asset = w.generate().rip(
            url=user.display_avatar.url, name=user.display_name, message="R.I.P"
        )
        dest = BytesIO(asset)
        dest.seek(0)
    ...
    async def hug(self, ctx, user: discord.User):
        await ctx.defer()
        im = w.generate().two_image(
            type="hug", url1=ctx.author.display_avatar.url, url2=user.display_avatar.url
        )
        dest = BytesIO(im)
        dest.seek(0)
    ...
    async def bed(self, ctx, user: discord.User):
        await ctx.defer()
        im = w.generate().two_image(
            type="bed", url1=ctx.author.display_avatar.url, url2=user.display_avatar.url
        )
        dest = BytesIO(im)
        dest.seek(0)
    ...
    async def batslap(self, ctx, user: discord.User):
        ...
        im = w.generate().two_image(
            type="batslap",
            url1=ctx.author.display_avatar.url,
            url2=user.display_avatar.url,
        )
        dest = BytesIO(im)
        dest.seek(0)
    ...
    async def friendship(self, ctx, user: discord.User):
        await ctx.defer()
        im = w.generate().friendship(
            url1=ctx.author.display_avatar.url,
            url2=user.display_avatar.url,
            text1=ctx.author.display_name,
            text2=user.display_name,
        )
        dest = BytesIO(im)
        dest.seek(0)
    ...
    async def tweet(self, ctx, text: str, *, user: discord.User = None):
        ...
        await ctx.defer()
        im = w.generate().tweet(
            url=user.display_avatar.url, name=user.display_name, text=text
        )
        dest = BytesIO(im)
        dest.seek(0)
    ...
    async def this_is_spotify(self, ctx, user: discord.User = None):
        user = ctx.author if user is None else user
        await ctx.defer()
        im = w.generate().spotify(
            url=user.display_avatar.url,
            hex=str(user.color).strip("#"),
    ...
    async def gayrate(self, ctx, user: discord.User = None):
        ...
        embed = discord.Embed(
            description=f"> Kanna Chan used her legendary powers to calculate **{user.display_name}'s** GayRate and found out that they are ||**{rate}%**|| gay.",
            color=ec,
        )
        embed.set_author(
    ...
    async def simprate(self, ctx, user: discord.User = None):
        ...
        rate = random.randint(0, 101)
        embed = discord.Embed(
            description=f"> Kanna Chan used her legendary powers to calculate **{user.display_name}'s** SimpRate and found out that they are ||**{rate}%**|| simp.",
            color=ec,
        )
        embed.set_author(
    ...
    @slash_command(name="kawaiirate", description="Why are you gay?")
    async def kawaiirate(self, ctx, user: discord.User = None):
        user = ctx.author if user is None else user
        rate = random.randint(0, 101)
        embed = discord.Embed(
            description=f"> Kanna Chan used her legendary powers to calculate **{user.display_name}'s** KawaiiRate and found out that they are ||**{rate}%**|| kawaii uwu.",
            color=ec,
        )
        embed.set_author(
