...
class Kazuki(commands.Cog):
    ...
    async def handle_ghost_ping(self, message):
        if len(message.mentions) > 0:
            ...
            if len(pinged_users) > 0:
                emb = discord.Embed(description=f"", color=ec)
                emb.add_field(name="User", value=message.author.name, inline=True)
                emb.add_field(
                    name="Users Pinged", value=pinged_users.rstrip(", "), inline=True
                )
                emb.add_field(name="Message", value=message.content, inline=False)
                emb.set_author(
                    name="Ghost Ping Found!!", icon_url=self.client.user.display_avatar
                )
    ...
    async def on_message_delete(self, message):
        if message.guild.id == self.kazuki and not message.author.bot:
            ...
            emb.set_author(
                name="Message Deleted!", icon_url=self.client.user.display_avatar
            )
            emb.timestamp = datetime.datetime.utcnow()
            await channel.send(embed=emb)
    ...
    async def on_message_edit(self, before, after):
        if before.guild.id == self.kazuki and not before.author.bot:
            ...
            emb.set_author(
                name="Message Edited!", icon_url=self.client.user.display_avatar
            )
            emb.timestamp = datetime.datetime.utcnow()
            await channel.send(embed=emb)
