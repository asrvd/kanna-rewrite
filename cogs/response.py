...
class SimpView(View):
    ...
    @discord.ui.button(label="Simp Back", style="2", emoji=discord.PartialEmoji(name="Kannawee", id=877036162122924072))
    async def y_callback(self, button, interaction):
        async with aiohttp.ClientSession() as cs:
            async with cs.get('https://getpickuplines.herokuapp.com/lines/random') as r:
                resp = await r.json()
                pl = resp['line']
                await interaction.response.send_message(f"> {interaction.user.mention} simps back on {self.auth.mention} <:cute_stare:882300914101289031>\n> `{pl}`")
    
    @discord.ui.button(label="Ignore", style="2", emoji=discord.PartialEmoji(name="kanna_no", id=877038775686021130))
...
class Response(commands.Cog):
    ...
    async def simp(self, ctx, user:discord.User):
        ...
        async with aiohttp.ClientSession() as cs:
            async with cs.get('https://getpickuplines.herokuapp.com/lines/random') as r:
                ...
                await ctx.respond(f"> {ctx.author.mention} simps on {user.mention} <:cute_stare:882300914101289031>\n> `{pl}`", view=view)
    ...
    async def dadjoke(self, ctx):
        ...
        await ctx.defer()
        async with aiohttp.ClientSession() as cs:
            async with cs.get('https://weebyapi.xyz/json/dadjoke', headers=headers) as r:
                resp = await r.json()
    ...
    async def roast(self, ctx, user:discord.User=None):
        ...
        async with aiohttp.ClientSession() as cs:
            async with cs.get('https://weebyapi.xyz/json/roast', headers=headers) as r:
                resp = await r.json()
                await ctx.followup.send(f"> *{ctx.author.mention} roasts {user.mention} (´。＿。｀)*\n> *`{resp['response']}`*")
...
class NResponse(commands.Cog):
    ...
    async def simp(self, ctx, user:discord.User):
        async with aiohttp.ClientSession() as cs:
            async with cs.get('https://getpickuplines.herokuapp.com/lines/random') as r:
                ...
                pl = resp['line']
                view = SimpView(ctx, ctx.author, user)
                msg = await ctx.send(f"> {ctx.author.mention} simps on {user.mention} <:cute_stare:882300914101289031>\n> `{pl}`", view=view)
                async def timeout():
    ...
    async def roast(self, ctx, user:discord.User=None):
        ...
        async with aiohttp.ClientSession() as cs:
            async with cs.get('https://weebyapi.xyz/json/roast', headers=headers) as r:
                resp = await r.json()
                await ctx.send(f"> *{ctx.author.mention} roasts {user.mention} (´。＿。｀)*\n> *`{resp['response']}`*")
